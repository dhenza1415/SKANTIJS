...
cl = LineClient(authToken="EwWEPeGigNfEbvRAEgdf.RcO+gtSFS9mgSS+yYZoNVW.u6Xc82LB4xYy0FUKJp0sWyNiX4AqLng1mYGDt4mq/M4=")
cl.log("Auth Token : " + str(cl.authToken))
ki = LineClient(authToken="Ew6Pe2VV62OnJGqv3p8a.y5VxpNSHKkly4g+m7/UdgG.lllCCuRLC29ZieutEialFmJLmsEpELpF4SUY1S4gYoc=")
ki.log("Auth Token : " + str(ki.authToken))
kk = LineClient(authToken="Ew30G2J961Q91HN0gW91.tymKwInKTygiyMM1IX4vuq.Pbofbdn3n74pv0jFKrzgkxEtUqp9D8aUP6Nu446WOvI=")
kk.log("Auth Token : " + str(kk.authToken))
kc = LineClient(authToken="Ewo2nZcmfvff5K8hLis4.XuQrezi1qIyJlRlvV0wxra.xXFuarIzoPjXKz94b4XuZBl9Wgurg608CKONEHidfBk=")
kc.log("Auth Token : " + str(kc.authToken))
kb = LineClient(authToken="Ewzy9FKQsGHgtiTOd2E2.UgaA6PtgDwrhl20KYKxh8G.4aQHlBC07J3Qw9MSOtvkfH0J2LkKu/XRCa5z5GKyCZM=")
kb.log("Auth Token : " + str(kb.authToken))
sw = LineClient(authToken="EwhrOskzXNjQH3rG5oWd.jB/U5D2EIoY5xGkQ7FOTtq.6n1iBTzIHz53z9y/I5OsQModMbdPm/yIgstlSNboIC0=")
            ...
            if mid in op.param3:
                if wait["autoLeave"] == True:
                    if op.param2 not in Bots and op.param2 not in owner and op.param2 not in admin and op.param2 not in staff:
                        ...
                        cl.acceptGroupInvitation(op.param1)
                        ginfo = cl.getGroup(op.param1)
                        cl.sendMessage(op.param1,"SK come back " + str(ginfo.name))
            ...
            if mid in op.param3:
                if wait["autoJoin"] == True:
                    if op.param2 not in Bots and op.param2 not in owner and op.param2 not in admin and op.param2 not in staff:
                        cl.acceptGroupInvitation(op.param1)
                        ginfo = cl.getGroup(op.param1)
                        cl.sendMessage(op.param1,"Kuii " +str(ginfo.name))
                    else:
                        cl.acceptGroupInvitation(op.param1)
                        ginfo = cl.getGroup(op.param1)
                        cl.sendMessage(op.param1,"Kuii " + str(ginfo.name))
            if Amid in op.param3:
                if wait["autoJoin"] == True:
                    if op.param2 not in Bots and op.param2 not in owner and op.param2 not in admin and op.param2 not in staff:
                        ...
                        ki.acceptGroupInvitation(op.param1)
                        ginfo = ki.getGroup(op.param1)
                        ki.sendMessage(op.param1,"Kuii " + str(ginfo.name))
            if Bmid in op.param3:
                if wait["autoJoin"] == True:
                    if op.param2 not in Bots and op.param2 not in owner and op.param2 not in admin and op.param2 not in staff:
                        ...
                        kk.acceptGroupInvitation(op.param1)
                        ginfo = kk.getGroup(op.param1)
                        kk.sendMessage(op.param1,"Kuii " + str(ginfo.name))
           ...
           try:
               if op.param1 in ghost:
                   if op.param2 not in Bots and op.param2 not in owner and op.param2 not in admin and op.param2 not in staff:
                        ...
                        invsend = 0
                        Ticket = cl.reissueGroupTicket(op.param1)
                        kb.acceptGroupInvitationByTicket(op.param1,Tickrt)
                        sw.acceptGroupInvitationByTicket(op.param1,Tickrt)
                        kb.kickoutFromGroup(op.param1,[op.param2])
                        sw.kickoutFromGroup(op.param1,[op.param2])
                        kb.leaveGroup(op.param1)
                        sw.leaveGroup(op.param1)
                                  ...
                                  try:
                                      ...
                                      try:
                                          if op.param3 not in wait["blacklist"]:
                                              cl.kickoutFromGroup(op.param1,[op.param2])
                                              cl.inviteIntoGroup(op.param1,[Zmid])
                                              cl.sendMessage(op.param1, "Jir songong itu antijs di cancle kapok w cipok")
                                      except:
                                          pass
              return
        if op.type == 32:
                          ...
                          if wait["selfbot"] == True:
                            if msg._from in admin:
                               try:
                                   ...
                                   cl.sendMessage(msg.to,"Sudah bersih boss gada pesan php...")
                               except:
                                   pass
...
#===========BOT UPDATE============#
                          ...
                          if wait["selfbot"] == True:
                            if msg._from in admin:
                                ...
                                cl.sendMessage(msg.to,"✒ SKPro bot\n\n"+ma+"\nTotal「%s」Bots" %(str(len(Bots))))
                          ...
                          if wait["selfbot"] == True:
                            if msg._from in admin:
                                ...
                                cl.sendMessage(msg.to,"✒ SKpro admin\n\nSuper admin:\n"+ma+"\nAdmin:\n"+mb+"\nStaff:\n"+mc+"\nTotal「%s」SK staff" %(str(len(owner)+len(admin)+len(staff))))
...
#===================KICK ALL MEMBER=====================#
                            ...
                            if msg._from in admin:
                             if msg.toType == 2:
                                 ...
                                 cl.sendText(msg.to,"⚠ DANGER ⚠")
                                 cl.sendText(msg.to,"Proses cleanse....")
                                 cl.sendText(msg.to,"SLAM DARI KAMI S̶̭̗̞̙̿͑̽̆̃̒į̷̙̝̦̤̜̗́̉ͅl̸̛͓͋͋͆̍ę̶͇̮̦̣̖̙̘̪̉n̸͍̦͉̖̟͚̗̣̍̓̽̅̚ť̴̙͋ ̷̨̳̠͎̮̘͇̀̅̀͒̈́͊̕͝T̸̡̯̗̩̺͉̑́͛̌̒ͅé̶̡̱̯̮̯̊̏́̀̃͜a̴̭͓̫͚̐́̂̍̂̊̋̚m̸̨̨̹͎͍̳̥͆̓͗̿͐͗͑̿̓͠ ̴̠͐̂B̷̛̳͎̫̻̫̯̣͓̲͋̀͋̋͊̈͗͑o̵̲̾̈́͒͗t̴̢͍̫̰̠̞͖͍̬̑̊̽͒́̈́͗ͅ")
                                 targets = []
                                 for g in gs.members:
                                 ...
                                 if targets == []:
                                     ...
                                     for target in targets:
                                           if not target in Bots and op.param2 not in owner and op.param2 not in admin and op.param2 not in staff:
                                             try:
                                                 klist = [cl,ki,kk,kc]
